...
for t in tick1 tick2 tick3 tick4 tick5 tick6 tick7 tick8;
  do
    /usr/bin/time -l ./ku/krakenuniq/krakenuniq --report-file ${file_path}/ku_${t}.tsv --output off --threads 10 --preload-size ${ps} -db ./ku/microbial_db ${file_path}/${t}.fastq.gz  >& data/match_ku_mb_${t}.log
  done
